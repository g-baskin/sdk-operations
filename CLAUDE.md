...
- `src/app/model-tester.tsx` is the client UI for sending text with either normal or streaming responses, keeping session history, compacting long histories, saving/loading/clearing conversations, and viewing/editing/deleting long-term memory facts in the local SQLite database partitioned by student name.
...
- `src/lib/ai-tools.ts` defines the function-tool registry; add new tools by adding one registry entry with a `definition`, `policy`, `validate`, and `run` handler. Tool calls are logged to stdout and SQLite, validated before execution, and blocked when marked destructive or confirmation-required.
- `src/lib/database.ts` owns the SQLite schema/store for users, conversations, conversation turns, memories, and tool-call logs.
- `src/app/api/conversations/route.ts`, `src/app/api/memories/route.ts`, and `src/app/api/tool-calls/route.ts` expose database-backed touchpoints for saved data.
...
- `SDK_OPERATIONS_DATABASE_PATH` chooses the local SQLite database path; default is `./data/sdk-operations.sqlite`.
