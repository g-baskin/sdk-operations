...
Every tool call is validated before execution and logged with `type: "ai_tool_call"`. Tools marked as requiring confirmation are blocked until explicit confirmation support is added, and destructive tools are never allowed to run autonomously. The model can call tools repeatedly in one request: `src/lib/ai-text.ts` feeds each tool result back to the model for another decision, capped at 4 tool-call rounds before forcing a final answer with tool use disabled.
